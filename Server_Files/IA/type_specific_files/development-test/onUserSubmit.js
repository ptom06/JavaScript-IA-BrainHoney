		...
		//if($(this).val() == "submit") {
			...
			for (var i=0; feedbackObjs.length > i; i++){
				...
				for (var j=0; feedbackObjs[i].matchExp.length > j; j++){
					...
					if(feedbackObjs[i].matchAll == true || feedbackObjs[i].matchAll == "true") {
						if (allTrue == false) {
			...
			if(anyFound) {
				...
				IsLog.c(typeof window['IA-Storage']["feedback"]);
				if(typeof window['IA-Storage']["feedback"] != "string")
					$("#feedback").text(window['IA-Storage']["feedback"][feedbackIndex]);
				else
					$("#feedback").text(window['IA-Storage']["feedback"]);
			} else {
